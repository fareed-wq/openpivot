...
def test_successful_ip(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_cidr_fallback(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
        return getattr(mock_rdap, "default_return", {"status": "error"})
    mock_rdap.side_effect = mock_rdap_effect
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_failure_isolation(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
        return getattr(mock_rdap, "default_return", {"status": "error"})
    mock_rdap.side_effect = mock_rdap_effect
    ...
    mock_rdap.default_return = {"status": "success", "source": "x", "data": {}}
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_rdap_unsupported(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_longest_prefix_match(mock_dns, mock_rdap):
    ...
    mock_rdap.default_return = {"status": "success", "source": "https://rdap.provider-b/ip/8.8.8.8", "data": {}}
    mock_dns.return_value = {"status": "success", "hostname": "dns.google"}
...
@patch('app.intelligence.ip._fetch_rdap')
def test_https_preference(mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [
...
@patch('app.intelligence.ip._fetch_rdap')
def test_cache_behavior(mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
        return getattr(mock_rdap, "default_return", {"status": "error"})
    mock_rdap.side_effect = mock_rdap_effect
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_privacy_filtering(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
        return getattr(mock_rdap, "default_return", {"status": "error"})
    mock_rdap.side_effect = mock_rdap_effect
...
@patch("app.intelligence.ip._fetch_rdap")
@patch("app.intelligence.ip._get_reverse_dns")
def test_rdap_failure_combinations(mock_dns, mock_rdap):
    def mock_rdap_effect(url, **kwargs):
        if "data.iana.org" in str(url): return {"status": "success", "data": {"services": [[["8.0.0.0/8"], ["https://rdap.arin.net/registry/"]]]}}
        return getattr(mock_rdap, "default_return", {"status": "error"})
    mock_rdap.side_effect = mock_rdap_effect
    ...
    # Both fail
    mock_rdap.default_return = {"status": "error"}
