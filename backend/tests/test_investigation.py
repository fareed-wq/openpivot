...
@patch("app.intelligence.investigation.collect_ct_assets")
...
@patch("app.intelligence.investigation.collect_ip_intelligence")
@patch("app.intelligence.investigation.collect_asn_intelligence_from_ip")
def test_ipv4_investigation_routing(mock_asn, mock_ip, mock_ct, mock_http, mock_tls, mock_rdap, mock_email, mock_dns):
...
@patch("app.intelligence.investigation.collect_http_metadata")
@patch("app.intelligence.investigation.collect_ct_assets")
def test_domain_investigation_partial(mock_ct, mock_http, mock_tls, mock_rdap, mock_email, mock_dns):
...
def test_correlation_exception(mock_corr, mock_ct, mock_http, mock_tls, mock_rdap, mock_email, mock_dns):
    mock_dns.return_value = {"status": "success", "domain": "example.com", "records": {}}
    mock_email.return_value = {"status": "success"}
    if "mock_ct" in locals(): mock_ct.return_value = {"status": "success"}
    mock_rdap.return_value = {"status": "success"}
