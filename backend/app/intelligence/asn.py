...
def _get_iana_asn_bootstrap() -> list:
    ...
    resp = _fetch_rdap(IANA_ASN_BOOTSTRAP_URL, connect_timeout=CONNECT_TIMEOUT, read_timeout=READ_TIMEOUT)
    ...
    for ranges, urls in data.get("services", []):
        ...
        if best_url:
            for r in ranges:
                parts = r.split('-')
                if len(parts) == 2:
                    new_cache.append((int(parts[0]), int(parts[1]), best_url))
                else:
                    new_cache.append((int(parts[0]), int(parts[0]), best_url))
