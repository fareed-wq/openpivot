...
def run_investigation(target_input: str) -> dict:
    ...
    try:
        if tgt_type == "domain":
            correlation_res = build_correlations(
                ...
                rdap_result=collectors_data.get("rdap"),
                http_result=collectors_data.get("http_metadata")
            )
        else:
            correlation_res = build_correlations(
    ...
    org_footprint = derive_organization_footprint(collectors_data, correlation_res)
    ...
    res = InvestigationResult(
        ...
        organization_footprint=org_footprint
    )
