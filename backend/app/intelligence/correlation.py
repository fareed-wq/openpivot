...
class CorrelationEngine:
    ...
    def add_relationship(self, source: str, target: str, rel_type: str, collector: str, attributes: Optional[Dict[str, Any]] = None):
        ...
        if rel_id not in self.relationships:
            ...
            if attributes:
                self.relationships[rel_id].attributes.update(attributes)
...
def build_correlations(
            ...
            if ns_data and isinstance(ns_data, dict):
                for ns in ns_data.get("values", []):
                    if ns:
                        ns = ns.lower().rstrip('.')
                        ns_id = f"host:{ns}"
                        engine.add_entity(ns_id, "hostname", ns)
            ...
            if mx_data and isinstance(mx_data, dict):
                for mx in mx_data.get("values", []):
                    ...
                    if host:
                        ...
                        mx_id = f"host:{host}"
                        engine.add_entity(mx_id, "hostname", host)
                        engine.add_relationship(domain_id, mx_id, "uses_mail_host", "dns")

    if rdap_result and rdap_result.get("status") == "success":
        domain = rdap_result.get("domain")
        if domain:
            domain_id = f"domain:{domain.lower()}"
            engine.add_entity(domain_id, "domain", domain)

            org = rdap_result.get("organization")
            if org:
                ...
                if org_id:
                    ...
                    engine.add_relationship(domain_id, org_id, "registered_to", "domain_rdap")
    ...
    if http_result and http_result.get("status") in ("success", "partial"):
        # We need the domain, but http_result doesn't store target domain.
        # However, we can use the main domain from domain_result.
        if domain_result and domain_result.get("domain"):
            domain = domain_result.get("domain").lower()
            domain_id = f"domain:{domain}"
            engine.add_entity(domain_id, "domain", domain)

            wf = http_result.get("web_footprint")
            if wf and wf.get("technologies"):
                for tech in wf["technologies"]:
                    tech_name = tech.get("name")
                    if tech_name:
                        tech_id = f"tech:{tech_name.lower().replace(' ', '_')}"

                        attrs = {}
                        if tech.get("category"): attrs["category"] = tech["category"]
                        engine.add_entity(tech_id, "technology", tech_name, attrs)

                        rel_attrs = {}
                        if tech.get("confidence"): rel_attrs["confidence"] = tech["confidence"]
                        if tech.get("evidence"): rel_attrs["evidence"] = tech["evidence"]

                        engine.add_relationship(domain_id, tech_id, "uses_technology", "web_footprint", rel_attrs)
